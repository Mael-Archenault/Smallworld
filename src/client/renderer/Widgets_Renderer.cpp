...
Widgets_Renderer::Widgets_Renderer(sf::RenderWindow& window) : window(window)
{
    ...
    start_conquest_button.set_content("Start conquests");
    start_conquest_button.set_character_size(font_size);
    start_conquest_button.set_position(sf::Vector2f(0.f, 0.f));
    start_conquest_button.set_size(sf::Vector2f(button_width, button_height));
    start_conquest_button.set_colors(sf::Color::White, sf::Color(50, 50, 50), sf::Color::White);
...
void Widgets_Renderer::render(state::Game_State& state)
{
    ...
    {
        start_conquest_button.set_size(sf::Vector2f(section_width * 4 / 6, 2 * section_height / 7));
        start_conquest_button.set_position(
            sf::Vector2f(position_x + section_width / 6, position_y + 1 * section_height / 7));

        start_conquest_button.render(window);
    ...
    {
        conquer_button.set_size(sf::Vector2f(section_width * 4 / 6, section_height / 3));
...
std::unordered_map<std::string, sf::FloatRect> Widgets_Renderer::get_layout()
{
    ...
    button_positions["start_conquest_button"] = start_conquest_button.get_rect();
