...
Tribe_Renderer::Tribe_Renderer()
{
    ...
    species_spritesheet = std::make_unique<sf::Texture>();
    power_spritesheet   = std::make_unique<sf::Texture>();
    texture             = std::make_unique<sf::RenderTexture>();

    if (!species_spritesheet->loadFromFile(std::string(RESOURCE_DIR) +
                                           "/assets/species_spritesheet.png"))
    {
        throw std::runtime_error("Tribe_Renderer::set_species: Failed to load species spritesheet");
    }
    if (!power_spritesheet->loadFromFile(std::string(RESOURCE_DIR) +
                                         "/assets/power_spritesheet.png"))
    {
        throw std::runtime_error("Tribe_Renderer::set_species: Failed to load power spritesheet");
    }

    species_spritesheet_indexing = open_indexing_file(
        std::string(RESOURCE_DIR) + "/assets/species_spritesheet_indexing.json", "species");
    power_spritesheet_indexing = open_indexing_file(
                                ...
                                bool is_in_decline)
{
    ...
    {
        // throw std::runtime_error("Tribe_Renderer::set_sprite: Species name not found in indexing:
        // " + species_name);
        species_name = "Blank";
    }
    if (power_spritesheet_indexing.find(power_name) == power_spritesheet_indexing.end())
    {
        // throw std::runtime_error("Tribe_Renderer::set_sprite: Power name not found in indexing: "
        // + power_name);
        power_name = "Blank";
    }
    ...
    sf::IntRect speciesFrameRect(frameX * frameWidth, frameY * frameHeight, frameWidth,
                                 frameHeight);
...
std::unordered_map<std::string, std::pair<int, int>> Tribe_Renderer::open_indexing_file(
    std::string file_name, std::string spritesheet_type)
{
