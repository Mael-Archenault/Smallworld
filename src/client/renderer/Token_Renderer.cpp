...
Token_Renderer::Token_Renderer(std::string token_type) {
    ...
    if (!spritesheet.loadFromFile(std::string(RESOURCE_DIR) + "/assets/" + token_type +
                                  "_spritesheet.png")) {
        throw std::runtime_error("Token_Renderer::set_species: Failed to load " + token_type +
                                 " spritesheet");
    }
    ...
    std::string file_name =
        std::string(RESOURCE_DIR) + "/assets/" + token_type + "_spritesheet_indexing.json";
...
void Token_Renderer::set_number(int n) { number.setString("x" + std::to_string(n)); }
...
void Token_Renderer::render(sf::RenderWindow &window, sf::Vector2f position, bool display_number) {
    ...
    this->number.setPosition(position.x + this->sprite.getGlobalBounds().width + 5,
                             position.y + this->sprite.getGlobalBounds().height / 2 -
                                 this->number.getGlobalBounds().height / 2);
    window.draw(this->number);
}

float Token_Renderer::get_sprite_width() { return (float)sprite_size.first; }

float Token_Renderer::get_sprite_height() { return (float)sprite_size.second; }

void Token_Renderer::color(sf::Color color_tint) { this->sprite.setColor(color_tint); }
};  // namespace renderer