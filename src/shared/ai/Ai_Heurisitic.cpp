...
std::unique_ptr<engine::Command> Ai_Heuristic::give_command_Conquer()
{
    ...
    std::pair<int, int> area_id_cost_to_attack = {attackable_area.at(0).first,
                                                  attackable_area.at(0).second};
    ...
    return std::make_unique<engine::Conquer_Command>(
        id, area_id_cost_to_attack.first, std::min(required_units, available_units),
