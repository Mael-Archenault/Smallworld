...
std::pair<int, std::vector<std::shared_ptr<engine::Command>>> Ai_Advanced::calcul_stack(
    state::Game_State new_state, std::shared_ptr<engine::Command> command = nullptr, int depth = 0,
    int default_money = 0, int default_free_units = 0)
{
    if (command != nullptr)
    {
    ...
    std::pair<float, std::vector<std::shared_ptr<engine::Command>>>
        current_best_node_value_command = {0, std::vector<std::shared_ptr<engine::Command>>()};
    ...
    for (std::pair<int, int> area : engine.get_state().get_conquest_prices(id))
    {
        ...
        auto new_command     = std::make_shared<engine::Conquer_Command>(
            id, area.first, std::min(required_units, available_units),
        ...
        {  // check if dice was needed, in which case the gain is multiplied by the probability
            float current_gain =
                new_state.inform_rewards(id);  // TODO check if the ponderation works properly
            new_node_value_command.first =
                current_gain + (new_node_value_command.first - current_gain) *
...
std::shared_ptr<engine::Command> Ai_Advanced::give_command_Start()
{
    ...
    if (state.get_free_units_number(id) <=
        state.get_redeployable_areas(id).size())  // less than half the starting troops remains
    {
