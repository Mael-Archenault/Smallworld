...
{
    {
        ...
        engine::Engine    engine({"Alice", "Bob"});

        // ---------------- error testing ------------------------------

        engine.update();  // empty queue

        BOOST_CHECK_EQUAL(engine.get_state_version_id(), 1);

        engine.add_command(std::make_shared<engine::Start_Conquest_Command>(0));  // no active tribe
        BOOST_CHECK_THROW(engine.update(), std::runtime_error);
        engine.remove_last_command();

        engine.add_command(
            std::make_shared<engine::Choose_Species_Command>(1, 0));  // not the player's turn
        BOOST_CHECK_THROW(engine.update(), std::runtime_error);
        engine.remove_last_command();

        engine.add_command(std::make_shared<engine::Choose_Species_Command>(
            0, 0));  // correct command for taking a tribe
        engine.update();

        engine.add_command(
            std::make_shared<engine::Redeploy_Command>(0, 0, 1));  // not the right turn phase
        BOOST_CHECK_THROW(engine.update(), std::runtime_error);
        engine.remove_last_command();

        // ---------------------- simulate game turns -------------------------

        engine = engine::Engine({"Alice", "Bob"});  // reset engine

        // choose species for player 0

        engine.add_command(
            std::make_shared<engine::Choose_Species_Command>(state.get_current_player().id, 0));
        engine.update();
        ...
        while (state.get_current_turn_phase() == state::Turn_Phase::CONQUER)
        {
            ...
            if (attackable_areas.empty())
            {
                ...
                engine.add_command(
                    std::make_shared<engine::End_Conquer_Command>(state.get_current_player().id));
                engine.update();
                state = state::Game_State(engine.get_state());
                break;
            }
            ...
            engine.add_command(std::make_shared<engine::Conquer_Command>(
                state.get_current_player().id, area_to_attack.first, area_to_attack.second, false));
            engine.update();
            state = state::Game_State(engine.get_state());
        }
        ...
        while (state.get_current_turn_phase() == state::Turn_Phase::REDEPLOY)
        {
            ...
            engine.add_command(
                std::make_shared<engine::Redeploy_Command>(player_id, redeployable_areas.at(0), 1));
        ...
        engine.add_command(
            std::make_shared<engine::Decline_Command>(state.get_current_player().id));
        engine.update();
        state = state::Game_State(engine.get_state());
        ...
        std::shared_ptr<engine::Command> command =
            std::make_shared<engine::Start_Conquest_Command>(state.get_current_player().id);
