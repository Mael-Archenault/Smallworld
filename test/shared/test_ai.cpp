...
{
    std::vector<state::Player_Type> ais             = {state::Random_AI, state::Heuristic_AI,
                                                       state::Advanced_AI};
    ...
    {
        std::cout << "Ai " << ais_names.at(i) << " won " << win_count.at(i) << " times."
                  << std::endl;
    }
}
...
int play_ai(std::vector<state::Player_Type> ais_type, std::vector<std::string> ais_names)
{
    ...
    while (!state.is_game_finished())
    {
        ...
        std::shared_ptr<engine::Command> command =
            ais.at(current_player_id)->give_command(state.get_current_turn_phase());
        ...
        std::cout << state.get_round() << std::endl;
    }
